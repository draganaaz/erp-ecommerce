...
import * as styles from "../styles/Grid.module.scss";
...
const GridHelper = ({ colCount, children, md }) => {
  ...
  const renderRows = () => {
    ...
    for (let row = 0; row < rowCount; row++) {
      rows.push(
        <Row key={row + 100} className={styles.Row}>
          {renderCols()}
        </Row>
      );
    }
  ...
  const renderCols = () => {
    ...
    for (let col = 0; col < colCount; col++) {
      if (index < children.length) {
        cols.push(
          <Col key={col} className={styles.Col} md={md}>
            {children[index]}
          </Col>
        );
        index++;
  ...
  return <Container className={styles.Container}>{buildGrid()}</Container>;
};
